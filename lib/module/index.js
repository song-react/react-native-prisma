...
var NativePrisma_default;
var init_NativePrisma = __esm(() => {
  NativePrisma_default = TurboModuleRegistry.getEnforcing("Prisma");
});
...
import { Prisma as Prisma2 } from "@prisma/client/extension";
var reactiveQueriesExtension = () => Prisma2.defineExtension((client) => {
  const subscribedQueries = {};
  const refreshSubscriptions = async () => {
    for (const key in subscribedQueries) {
      const subscription = subscribedQueries[key];
...
var PrismaModule = isTurboModuleEnabled ? (init_NativePrisma(), __toCommonJS(exports_NativePrisma)).default : NativeModules.Prisma;
if (!PrismaModule) {
